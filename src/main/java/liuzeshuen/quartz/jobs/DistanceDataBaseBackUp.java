package liuzeshuen.quartz.jobs;

import liuzeshuen.quartz.dao.JdbcBeanMapper;
import liuzeshuen.quartz.model.JdbcBean;
import liuzeshuen.quartz.service.BaseJob;
import liuzeshuen.quartz.tool.SpringUtil;
...
public class DistanceDataBaseBackUp implements BaseJob {
    ...
    public void backup(JdbcBean jdbcBean) {
        try {
            ...
            if (exportFlag) {
                logger.info("远程备份数据库成功！,数据库为【{JdbcBean}】", jdbcBean.getDB());
            } else {
                logger.info("数据库数据备份失败,数据库为【{JdbcBean}】\", jdbcBean.getDB()");
            }
        } catch (Exception e) {
            logger.info("运行异常，数据库数据备份失败！");
            e.printStackTrace();
            logger.info("远程备份数据库失败,数据库为【{JdbcBean}】", jdbcBean.getDB());
        }
    }
...
//        String C="C:\\Program Files\\MySQL\\MySQL Server 5.6\\bin\\";
    ...
    public static String getFilePath(String fileName, JdbcBean jdbcBean) {
        String rootPath;
        String filPath;
        rootPath = jdbcBean.getBackupPath() + "distance/" + jdbcBean.getDB() + "/";
    ...
    public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
        List<JdbcBean> jdbcBeanList = SpringUtil.getBean(JdbcBeanMapper.class).selectDistance();
